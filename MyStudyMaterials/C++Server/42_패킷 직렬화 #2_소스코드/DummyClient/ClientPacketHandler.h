...
public:
	...
	// Iterator 와 range-based for 지원을 위한 연산자
	bool						operator!=(const PacketIterator& other) const { return _index != other._index; }
	const T&					operator*() const { return _container[_index]; }
	T&							operator*() { return _container[_index]; }
	T*							operator->() { return &_container[_index]; }
	PacketIterator&		operator++() { _index++; return *this; }// 전위 증가
	PacketIterator			operator++(int32) { PacketIterator ret = *this; ++_index; return ret; }// 후위 증가
