...
Memory::Memory()
{
	...
	- 32 byte, 64 byte, 96 byte, 128 byte... 1024 byte 단위 사이즈의
	 Memory Pool들을 각각 생성한다.
	 
	 예를들면
	 32 byte 단위 사이즈 Memory Pool은 1~32 byte 단위 크기의 메모리를 할당해주고,
	 64 byte 단위 사이즈 Memory Pool은 33~64 byte 단위 크기의 메모리를 할당해주는 방식이다.
	*/
	...
	for (; size <= 2048; size += 128) // 메모리 풀 사이즈 128씩 증가
	{
		MemoryPool* pool = new MemoryPool(size);
		pools.push_back(pool);
	...
	for (; size <= MAX_ALLOC_SIZE; size += 256) // 메모리 풀 사이즈 256씩 증가
	{
		MemoryPool* pool = new MemoryPool(size);
		pools.push_back(pool);
