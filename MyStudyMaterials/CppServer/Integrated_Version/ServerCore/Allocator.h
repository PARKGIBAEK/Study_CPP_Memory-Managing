...
/*
 STL Container(std::vector, std::queue ...) 사용 시 메모리 할당/해제 관련 부분을 기본 Allocator를 사용하지 않고 직접 구현한 Allocator를 사용하기 위해 만든 클래스.
 
  커스텀 Allocator를 사용할 때 할당/해제 관련 함수 네이밍은 allocate와 deallocate로 맞춰야한다.
  왜냐하면 STL Container들은 템플릿으로 구현되어 있으며, STL Container들의 할당/해제 구현부를 살펴보면 Allocator.allocate() 와 같은 방식으로 구현되어있기 때문이다.
  이때 템플릿 인자로 전달받은 Allocator가 allocate/deallocate라는 멤버 함수를 포함하지를 않더라도 IntelliSense가 템플릿 에러까지 미리 알아내지 못하기 때문에 빌드 시도 전에는 compile error가 발생할 것을 예상할 수 없다.
*/
template<typename T>
class STL_Allocator
{
