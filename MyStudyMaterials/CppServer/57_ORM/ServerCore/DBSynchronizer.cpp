...
namespace SP
{
	// Table이나 Column 정보들을 모두 긁어오는 쿼리
	...
	/* DBSynchronizer::GatherDBTables 함수에서 생성하여 사용 됨
	BindCol을 통해 0번~12번까지의 인자들을 바인딩하고, 위의 쿼리를 실행하여 DB의 모든 Table과 각Table의 Column정보를 가져온다*/
...
bool DBSynchronizer::GatherDBTables()
{
	...
	//DB의 Table과 Column정보를 가져오기 위해 필요한 인자들을 바인딩
	...
	//미리 정의된 쿼리(Table과 Column정보를 가져오기 위함) 실행
