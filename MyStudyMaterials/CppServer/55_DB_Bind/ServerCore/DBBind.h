...
// 데이터를 Write하는경우 받아올 인자가 없으므로 ColumnCount는 0이 된다.
...
protected:
	...
	const WCHAR*	_query;//실행할 쿼리
	SQLLEN			_paramIndex[ParamCount > 0 ? ParamCount : 1];//인자가 없을 경우 생길 수 배열길이가 0이되는 문법적 오류를 방지
