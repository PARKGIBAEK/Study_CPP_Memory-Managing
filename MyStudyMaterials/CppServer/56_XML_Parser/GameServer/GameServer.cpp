...
int main()
{
	...
	XmlNode root;
	XmlParser parser;
	if (parser.ParseFromFile(L"GameDB.xml", OUT root) == false)//Root Node는 <GameDB>부분
		return 0;

	Vector<XmlNode> tables = root.FindChildren(L"Table");//root노드의 하위 노드 중 <Table>이라는 이름을 가진 노드를 다 찾아 줌
