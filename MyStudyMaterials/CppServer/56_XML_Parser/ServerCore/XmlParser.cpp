...
{
	XmlAttributeType* attr = _node->first_attribute(key);//_node에서 key 값에 해당하는 attribute를 반환
	if (attr)
		//return ::_wcsicmp(attr->value(), L"true") == 0;//문자열 일치 비교 winapi
		return std::wcscmp(attr->value(), L"true") == 0; //c++표준
